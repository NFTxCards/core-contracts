...
describe("Exchange contract", function () {
    ...
    describe("Order", () => {
        it("should be success: create & fill order", async function () {
            ...
            // @ts-ignore
            const result = await auction.callStatic.getOrderFromHash(
                order.events![0].args!.orderHash,
            );
            ...
            expect(senderBalanceBefore.add(tokenPrice).toString()).to.be.eq(
        ...
        it("should be failed: only owner token can create order", async function () {
            ...
            ).to.be.revertedWith("Only owner token can create order");
        });
        ...
        it("should be failed: not enough money", async function () {
            ...
            await expect(
                auction.connect(recipient).fillOrder(result, {
